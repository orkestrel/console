import type { Attribute, CaptureLevel, Color, RendererInterface, Style } from '@src/core'
import {
	createCaptureResult,
	createConsoleSink,
	createStyler,
...
// createStyler / createTheme — the styling factories. createStyler returns the fluent surface,
// defaulting to ANSI + enabled, with options to swap the renderer (the C-f browser seam) and to
// disable. createTheme merges per role over DEFAULT_THEME and deep-freezes every style leaf.
// createConsoleSink is the C-b logging factory: the snapshotting, level-routing console sink.
...
describe('createTheme', () => {
	...
	it('snapshots a supplied level style before a live logger consumes it', () => {
		...
		const logger = new Logger({ level: 'debug', sink, theme })
	...
	it('snapshots a frozen status style record whose attributes remain mutable', () => {
		...
		const sink = createRecordingSink()
		const reporter = new Reporter({ sink, theme })
	...
	it('snapshots a supplied accent style before a live spinner consumes it', () => {
		...
		const sink = createRecordingSink()
		const spinner = new Spinner({ frames: ['*'], message: 'kept', sink, theme })
		style.foreground = 'magenta'
		spinner.tick()
	...
	it('captures a chrome foreground getter before a live reporter consumes it', () => {
		...
		const sink = createRecordingSink()
		const reporter = new Reporter({ sink, theme, width: 8 })
		foreground = 'magenta'
		reporter.section('S')
...
// createCaptureResult — the C-d scoped console-interception factory. It starts a `Capture`, runs
// `fn`, stops in a finally (so console is restored even on throw / reject), and returns
// { value, messages } — sync for a sync `fn`, a Promise for an async one. The `Capture` class it
// drives is covered in Capture.test.ts. Every test snapshots + restores console so a patch never
// leaks.

describe('createCaptureResult', () => {
	...
	describe('createCaptureResult — sync', () => {
	...
	describe('createCaptureResult — boundary shapes', () => {
		it('a sync fn that logs nothing yields an empty messages list and its value', () => {
			const { value, messages } = createCaptureResult(() => 7)
			expect(value).toBe(7)
		...
		it('preserves a falsy sync return value (0 / false / null / undefined)', () => {
			expect(createCaptureResult(() => 0).value).toBe(0)
			expect(createCaptureResult(() => false).value).toBe(false)
			expect(createCaptureResult(() => null).value).toBeNull()
			expect(createCaptureResult(() => undefined).value).toBeUndefined()
		})
		...
		it('the async overload resolves to a Promise even when fn returns a resolved promise of a value', async () => {
			const result = createCaptureResult(() => Promise.resolve('async value'))
			expect(result).toBeInstanceOf(Promise)
			const { value, messages } = await result
