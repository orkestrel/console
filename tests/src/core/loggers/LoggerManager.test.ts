...
// LoggerManager — the event-free registry of named loggers + a convenience fan-out. It
// mints + stores loggers keyed by name (its defaults flowing in unless overridden), looks
// them up, removes them (all / one / a batch under one verb), and broadcasts a one-off log
// to every logger.
...
describe('LoggerManager', () => {
	describe('registry — mint, store, look up, count', () => {
	...
	describe('removal — all, one, or a batch under one verb', () => {
	...
	describe('overwrite + removal interplay', () => {
		...
		it('remove([]) of an empty list removes nothing and returns true (every member succeeded)', () => {
			const { manager } = createTestManager()
			manager.register('a')
		...
		it('remove([names]) returns false when any name was absent, still removing the present ones', () => {
			const { manager } = createTestManager()
			manager.register('a')
			expect(manager.remove(['a', 'absent'])).toBe(false) // one absent ⇒ the batch did not succeed
			expect(manager.count).toBe(0) // every present name was still removed
		})
	})
